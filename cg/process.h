...
namespace CG {
	...
	public:
		...
		static bool Open(std::wstring file, LPCWSTR args, UINT show = SW_NORMAL, LPCWSTR workPath = 0) {
		...
		/* cmdLine: "C:\A\1.exe" -p */
		static bool Open(std::wstring cmdLine, UINT show = SW_NORMAL, LPCWSTR workPath = 0) {
		...
		/* file: "C:\A\1.exe", args: -p */
		static bool Start(std::wstring file, LPCWSTR args, UINT show = SW_NORMAL, LPCWSTR workPath = 0, DWORD creationFlags = CREATE_NEW_CONSOLE | CREATE_UNICODE_ENVIRONMENT)
		{
		...
		static bool StartWithSystem(LPCWSTR cmdLine, LPCWSTR workPath = 0)
		{
			DWORD pid = 0;
			PROCESSENTRY32W pe = {};
			pe.dwSize = sizeof(PROCESSENTRY32W);
				...
				if (pid)
				{
					...
					if (hProcess)
					{
						...
						if (hToken)
						{
							...
							STARTUPINFOW si = {};
							si.cb = sizeof(STARTUPINFOW); si.wShowWindow = SW_SHOW;
							PROCESS_INFORMATION pi = {};
							BOOL b = 0;
							if (workPath) CreateProcessWithTokenW(hToken, LOGON_NETCREDENTIALS_ONLY, 0, (LPWSTR)cmdLine, NORMAL_PRIORITY_CLASS, 0, workPath, &si, &pi);
							else CreateProcessWithTokenW(hToken, LOGON_NETCREDENTIALS_ONLY, 0, (LPWSTR)cmdLine, NORMAL_PRIORITY_CLASS, 0, Path::RemoveFile(Path::RemoveArgs(cmdLine)).c_str(), &si, &pi);
							CloseHandle(hToken);
							if (b)
							{
								CloseHandle(pi.hThread);
