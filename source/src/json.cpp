...
namespace QiJson
{
	...
	void SaveAction(neb::CJsonObject& jActions, const Actions& actions)
	{
		for (uint32 u = 0; u < actions.size(); u++)
		{
			...
			jItem.Add("type", action.base.type);
			jItem.Add("mark", String::toString(action.base.mark));

			switch (action.base.type)
			{
			case QiType::delay:
			{
				jItem.Add("ms", action.delay.min);
				jItem.Add("ex", action.delay.max);
				break;
			}

			case QiType::key:
			{
				jItem.Add("state", action.key.state);
				jItem.Add("vk", action.key.vk);
				break;
			}

			case QiType::mouse:
			{
				jItem.Add("move", action.mouse.move, true);
				jItem.Add("x", action.mouse.x);
				jItem.Add("y", action.mouse.y);
				jItem.Add("ex", action.mouse.ex);
				jItem.Add("trk", action.mouse.track, true);
				jItem.Add("spd", action.mouse.speed);
				break;
			}

			case QiType::text:
			{
				jItem.Add("text", String::toString(action.text.str));
				break;
			}

			case QiType::color:
			{
				jItem.Add("unfind", action.color.unfind, true);
				jItem.Add("move", action.color.move, true);
				jItem.Add("left", (int32)action.color.rect.left);
				jItem.Add("top", (int32)action.color.rect.top);
				jItem.Add("right", (int32)action.color.rect.right);
				jItem.Add("bottom", (int32)action.color.rect.bottom);
				jItem.Add("rgbe", (uint32)action.color.rgbe.toCOLORREF());
				SaveAction(jNext, action.color.next), jItem.Add("next", jNext);
				break;
			}

			case QiType::loop:
			{
				jItem.Add("count", action.loop.min);
				jItem.Add("rand", action.loop.max);
				SaveAction(jNext, action.loop.next), jItem.Add("next", jNext);
				break;
			}

			case QiType::keyState:
			{
				jItem.Add("state", action.keyState.state, true);
				jItem.Add("vk", action.keyState.vk);
				SaveAction(jNext, action.keyState.next), jItem.Add("next", jNext);
				break;
			}

			case QiType::image:
			{
				jItem.Add("unfind", action.image.unfind, true);
				jItem.Add("move", action.image.move, true);
				jItem.Add("left", (int32)(action.image.rect.left));
				jItem.Add("top", (int32)(action.image.rect.top));
				jItem.Add("right", (int32)(action.image.rect.right));
				jItem.Add("bottom", (int32)(action.image.rect.bottom));
				jItem.Add("sim", action.image.sim);
				jItem.Add("width", action.image.map.width());
				jItem.Add("height", action.image.map.height());
				std::string data(Base64::EncodedLength(action.image.map.bytes()), '\0');
				if (Base64::Encode((const char*)action.image.map.data(), action.image.map.bytes(), &data[0], data.size())) jItem.Add("data", data);
				SaveAction(jNext, action.image.next), jItem.Add("next", jNext);
				break;
			}

			case QiType::popText:
			{
				jItem.Add("text", String::toString(action.popText.str));
				jItem.Add("time", action.popText.time);
				break;
			}

			case QiType::timer:
			{
				jItem.Add("min", action.timer.min);
				jItem.Add("max", action.timer.max);
				SaveAction(jNext, action.timer.next), jItem.Add("next", jNext);
				break;
			}
