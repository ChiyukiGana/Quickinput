...
namespace QiTools
{
	...
	public:
		...
		map2d(const size_type width, const size_type height, const ty& init = ty()) noexcept { assign(width, height, init); }
		...
		void assign(const size_type width, const size_type height) noexcept { if (width && height) { if (m_w != width && m_h != height) m_w = width, m_h = height, vector::assign(width * height, ty()); } else clear(); }
		void assign(const size_type width, const size_type height, const ty& init) noexcept { if (width && height) m_w = width, m_h = height, vector::assign(width * height, init); else clear(); }
		...
		size_type width() const noexcept { return m_w; }
		size_type height() const noexcept { return m_h; }
		size_type bytes() const noexcept { return size() * sizeof(ty); }

		ty& at(const size_type x, const size_type y) { return at(m_w * y + x); }
		const ty& at(const size_type x, const size_type y) const { return at(m_w * y + x); }
	};
}