...
protected:
	...
	virtual void mousePressEvent(QMouseEvent* e)
	{
		if (e->button() == Qt::LeftButton)
		{
			...
			bool widthResizable = (maximumWidth() - minimumWidth() > 0);
			bool heightResizable = (maximumHeight() - minimumHeight() > 0);
			if (m_window_adjust_scale_width > 0 && (widthResizable || heightResizable))
			{
				...
				if (r && b && widthResizable && heightResizable) m_window_adjust_type = 5;
				else if (l && b && widthResizable && heightResizable) m_window_adjust_type = 6;
				else if (r && t && widthResizable && heightResizable) m_window_adjust_type = 7;
				else if (l && t && widthResizable && heightResizable) m_window_adjust_type = 8;
				else if (r && widthResizable) m_window_adjust_type = 2;
				else if (b && heightResizable) m_window_adjust_type = 3;
				else if (l && widthResizable) m_window_adjust_type = 4;
				else if (t && heightResizable) m_window_adjust_type = 9;
			}

			if (m_window_adjust_type == 0)
			{
				if (m_window_adjust_move_height < 1) m_window_adjust_type = 1;
				else
				{
					QRect move_rect(0, 0, width(), m_window_adjust_move_height);
					if (move_rect.contains(e->pos())) m_window_adjust_type = 1;
				}
			}
			e->accept();
		}
...
private:
	void updateCursorShape()
	{
		...
		if (m_window_adjust_scale_width > 0 && (widthResizable || heightResizable))
		{
			...
			if (r && b && widthResizable && heightResizable) setCursor(Qt::SizeFDiagCursor);
			else if (l && b && widthResizable && heightResizable) setCursor(Qt::SizeBDiagCursor);
			else if (r && t && widthResizable && heightResizable) setCursor(Qt::SizeBDiagCursor);
			else if (l && t && widthResizable && heightResizable) setCursor(Qt::SizeFDiagCursor);
			else if (r && widthResizable) setCursor(Qt::SizeHorCursor);
			else if (b && heightResizable) setCursor(Qt::SizeVerCursor);
			else if (l && widthResizable) setCursor(Qt::SizeHorCursor);
			else if (t && heightResizable) setCursor(Qt::SizeVerCursor);
			else unsetCursor();
		}
		else unsetCursor();
	}
